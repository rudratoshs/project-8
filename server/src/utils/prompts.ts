...
export function generateCoursePrompt(data: CreateCourseData, topicIndex: number): string {
    ...
    return `
          ...
          "subtopics": [
            {
              ...
              ${isCurrentTopic ? '"theory": "Detailed content explaining the subtopic, including examples.",' : '"theory": "",'}
              "youtube": "",
              "image": "",
      ...
      - ${isCurrentTopic ? 'Provide detailed content in the "theory" field for each subtopic, including relevant examples.' : 'Leave the "theory" field empty for subtopics.'}
      - Ensure the explanations are concise, clear, and educational.
      - Keep the "youtube" and "image" fields empty.
      - Set the "done" field to ${isCurrentTopic ? 'true' : 'false'} for all subtopics.
  
      ${data.subtopics?.length
            ? `Additional Requirement:
      - Include references to these subtopics if relevant: ${data.subtopics.map(subtopic => `"${subtopic}"`).join(', ')}.`
            : ''
        }
    `.trim();
}
...
export function generatePreviewPrompt(data: CreateCourseData): string {
    const subtopicsList = data.subtopics?.length ? data.subtopics.join(", ") : "none";
...
export function generateSubtopicDetailsPrompt(mainTopic: string, subTopic: string): string {
    return `
...
export function generateSubtopicContentPrompt(mainTopic: string, subTopic: string): string {
    return `
