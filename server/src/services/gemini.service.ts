...
import { generateCoursePrompt, generatePreviewPrompt, generateSubtopicContentPrompt } from '../utils/prompts';
...
export class GeminiService {
   ...
   */
  async generateCourseContent(data: CreateCourseData, currentTopicIndex: number = 0) {
  ...
  async handleFailedSubtopics(courseId: string, topicId: string): Promise<any> {
    ...
    const failedSubtopics = topic.subtopics.filter(
      (sub) => sub.content === 'Content generation failed due to invalid response format.'
    );
    ...
    await Promise.all(
      failedSubtopics.map(async (subtopic) => {
        ...
        subtopic.content = parsedContent.content || 'Content generation failed.';
        subtopic.status = 'complete';
      })
    );
